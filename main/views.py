...
from usuarios.models import Usuario, Votacion
...
def detail(request, idp):
	...
	votos = len(get_list_or_404(Votacion, producto=producto))
	if request.user.is_authenticated():
		usuario = Usuario.objects.get(uid=request.user.username)
		return render_to_response('detail.html', {
			'prod':producto, 'capturas':capturas, 'usuario': usuario, 'votos':votos}, context_instance=RequestContext(request))
	else:
		return render_to_response('detail.html', {
